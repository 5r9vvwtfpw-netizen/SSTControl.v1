...
- **H04 - Vehículos Seguros** → botón abre '/pesv/evaluacion/:id/vehiculos' (también accesible desde PESV → Vehículos en el menú lateral → '/pesv/vehiculos') — Inventario de vehículos: placa, marca, modelo, año, tipo, SOAT (vencimiento), revisión tecnicomecánica (vencimiento), tarjeta de operación.
  **PASOS PARA VER LAS FECHAS DE SOAT Y TECNICOMECÁNICA DE TODA LA FLOTA:**
  Paso 1 → Clic en la pestaña **"PESV"** en la barra de navegación superior.
  Paso 2 → Clic en **"Evaluaciones PESV"** en el menú desplegable.
  Paso 3 → En la lista de evaluaciones, clic en la evaluación activa del año en curso (botón **"Ir a evaluación"** o el nombre).
  Paso 4 → Dentro de la evaluación, localizar el paso **"H04 - Vehículos Seguros"** en la fase **Hacer**.
  Paso 5 → Clic en el botón **"Ir al módulo"** del paso H04.
  Paso 6 → Se abre la tabla de vehículos. Allí aparecen las columnas **"Venc. SOAT"** y **"Venc. Tecnicomecánica"** con badge de color para cada vehículo: 🔴 rojo = vencido, 🟡 amarillo = vence en ≤30 días, 🟢 verde = vigente, "Sin registro" = no se ha ingresado la fecha. No es necesario abrir cada vehículo individualmente — toda la flota se ve de un vistazo.
  **PASOS PARA DESCARGAR EL INFORME PDF DE VENCIMIENTOS:**
  Paso 1 a 5 → Igual que arriba (llegar al módulo H04 desde la evaluación).
  Paso 6 → En la parte superior derecha de la pantalla, clic en el botón **"Descargar informe PDF"**.
  Paso 7 → El sistema descarga un PDF con dos tablas: (1) Parque automotor (placa, marca, modelo, tipo, estado) y (2) tabla **"Vencimientos de Documentos"** con Placa | Venc. SOAT | Estado SOAT | Venc. Rev. Técnica | Estado RTM. Filas con "⚠ Vencido" = documento ya vencido. Útil para auditorías, correos o impresión.
  **ALERTAS AUTOMÁTICAS (el sistema las envía solo, sin pasos adicionales):** Todos los días a las 8:00 AM el sistema revisa la flota y los conductores. Si detecta SOAT, RTM, licencia de conducción o examen médico vencido o próximo a vencer (≤30 días), envía una notificación interna (campanita 🔔) y un correo al administrador de la empresa. REQUISITO IMPORTANTE: para que las alertas funcionen, cada vehículo debe tener registradas las fechas en su ficha (campos "Vencimiento SOAT" y "Vencimiento Tecnicomecánica") — si están vacíos, el sistema no puede generar la alerta.
