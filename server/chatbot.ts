...
- **P03 - Diagnóstico / Contexto Organizacional** → **EXCLUSIVO DEL LSO ASIGNADO.** La empresa no lo diligencia. Para que el LSO pueda gestionarlo correctamente, la empresa debe tener diligenciados los siguientes módulos: **H04 - Vehículos Seguros** (flota con SOAT y RTM), **H01 - Factor Humano / Conductores** (conductores activos con licencia y examen médico), **H03 - Comparendos** (infracciones de cada conductor en H01), **H02 - Capacitación en Seguridad Vial** (capacitaciones del año), **V02 - Registro y Análisis de Siniestros** (siniestros del año), y el módulo **Trabajadores** del SG-SST. Orden recomendado: H04 → H01 → H03 → H02 → V02 → Trabajadores → luego el LSO gestiona P03.
...
- **P05 - Objetivos e Indicadores (Indicadores SPI)** → **EXCLUSIVO DEL LSO ASIGNADO.** La empresa no lo diligencia. Para que el LSO pueda gestionarlo correctamente, la empresa debe tener diligenciados: **H04 - Vehículos Seguros** (flota con SOAT y RTM), **H01 - Factor Humano / Conductores** (conductores activos con fechas de licencia y examen médico), **H03 - Comparendos** (infracciones de cada conductor), **H02 - Capacitación en Seguridad Vial** (capacitaciones del año), **H05 - Mantenimiento Vehicular** (mantenimientos del año), **H06 - Inspecciones Preoperacionales** (inspecciones realizadas), **V02 - Registro y Análisis de Siniestros** (siniestros del año). Orden recomendado: H04 → H01 → H03 → H02 → H05 → H06 → V02 → luego el LSO gestiona P05.
- **P06 - Programas y Planes / Factores de Desempeño SPF** → **EXCLUSIVO DEL LSO ASIGNADO.** La empresa no lo diligencia. Para que el LSO pueda gestionarlo correctamente, la empresa debe tener diligenciados: **H04 - Vehículos Seguros** (flota con SOAT y RTM), **H01 - Factor Humano / Conductores** (conductores activos con fechas de licencia y examen médico), **H03 - Comparendos** (infracciones de cada conductor), **H02 - Capacitación en Seguridad Vial** (capacitaciones del año), **H05 - Mantenimiento Vehicular** (mantenimientos del año), **H06 - Inspecciones Preoperacionales** (inspecciones realizadas), **V02 - Registro y Análisis de Siniestros** (siniestros del año con heridos y fallecidos). Orden recomendado: H04 → H01 → H03 → H02 → H05 → H06 → V02 → luego el LSO gestiona P06.
...
- **H04 - Vehículos Seguros** → botón abre '/pesv/evaluacion/:id/vehiculos' (también accesible desde PESV → Vehículos en el menú lateral → '/pesv/vehiculos') — Inventario de vehículos: placa, marca, modelo, año, tipo, SOAT (vencimiento), revisión tecnicomecánica (vencimiento), tarjeta de operación.
  **PASOS PARA LLEGAR AL MÓDULO DE VEHÍCULOS (H04):**
  Paso 1 → Clic en la pestaña **"PESV"** en la barra de navegación superior.
  Paso 2 → Clic en **"Evaluaciones PESV"** en el menú desplegable. Se abre la lista de evaluaciones en '/pesv/evaluaciones'.
  Paso 3 → Clic en el botón **"Ir a evaluación"** (o en el nombre) de la evaluación activa del año en curso.
  Paso 4 → Dentro de la evaluación, buscar el paso **"H04 - Vehículos Seguros"** en la fase Hacer.
  Paso 5 → Clic en el botón **"Ir al módulo"** del paso H04. Se abre la tabla de vehículos en '/pesv/evaluacion/:id/vehiculos'.
  (Atajo directo: pestaña PESV → "Vehículos" en el menú desplegable, llega directo a la tabla sin pasar por la evaluación.)
  **CÓMO VER LAS FECHAS DE SOAT Y TECNICOMECÁNICA SIN ABRIR CADA VEHÍCULO:** Una vez en la tabla de vehículos, ya aparecen dos columnas con badge de color para toda la flota: **"Venc. SOAT"** y **"Venc. Tecnicomecánica"**. Los colores significan: 🔴 rojo = documento vencido, 🟡 amarillo = vence en los próximos 30 días, 🟢 verde = vigente, "Sin registro" = no se ha ingresado la fecha. No es necesario abrir cada vehículo individualmente.
  **PASOS PARA GENERAR EL INFORME PDF CON FECHAS DE SOAT Y RTM:**
  Paso 1 → Llegar al módulo de vehículos (H04) siguiendo los pasos anteriores.
  Paso 2 → En la parte superior derecha de la pantalla, clic en el botón **"Descargar informe PDF"**.
  Paso 3 → El sistema descarga automáticamente un PDF con dos tablas: (1) Parque automotor (placa, marca, modelo, tipo, estado) y (2) **"Vencimientos de Documentos"** con Placa | Venc. SOAT | Estado SOAT | Venc. Rev. Técnica | Estado RTM. Las filas con "⚠ Vencido" indican documentos ya vencidos. Este PDF sirve para auditorías, correos o impresión.
  **ALERTAS AUTOMÁTICAS DE VENCIMIENTO (sin hacer nada — el sistema lo hace solo):** El sistema envía automáticamente una notificación interna (campanita 🔔) y un correo al administrador o responsable SST **todos los días a las 8:00 AM** cuando detecta: SOAT o RTM vencidos o que vencen en ≤30 días, licencias de conducción de conductores vencidas o próximas a vencer, exámenes médicos de conductores vencidos o próximos a vencer. Requisito: cada vehículo debe tener registradas las fechas de vencimiento en su ficha (campos "Vencimiento SOAT" y "Vencimiento Tecnicomecánica"). Si no están registradas, el sistema no puede alertar. La alerta no se repite si ya se envió en los últimos 7 días.
...
**Fase ACTUAR (A01-A02):**
- **A01 - Mejora Continua** → gestionado directamente desde la página del paso A01 dentro de la evaluación PESV (pestaña "Actuar") — Registro de acciones de mejora derivadas de siniestros, auditorías, indicadores e inspecciones (Art. 20 Res. 40595/2022). Para crear una acción: clic en **"Nueva Acción"** → se abre el formulario **Smart Form** (con auto-llenado inteligente) → campos del formulario: **Descripción** * (qué se va a hacer — único campo obligatorio), **Tipo de Acción** (selección: Correctiva / Preventiva / Mejora), **Prioridad** (selección: Baja / Media / Alta / Crítica), **Fuente del Hallazgo** (selección: Auditoría / Indicador / Siniestro / Inspección / Revisión por la Dirección — indica de dónde surge la acción), **Responsable** (nombre del responsable — se **auto-llena automáticamente** con el nombre del usuario activo, badge azul "Usuario activo"), **Fecha Límite** (fecha para completar — se **auto-llena automáticamente** con 30 días a partir de hoy, badge ámbar "30 días"), **Observaciones** (texto adicional opcional). **IMPORTANTE: el campo "Estado" NO aparece en el formulario de creación** — las acciones nuevas quedan siempre en estado "Pendiente" al crearse. El formulario también muestra un **panel de sugerencias** (panel ámbar con ícono de bombilla) cuando la evaluación tiene pasos marcados como "No Cumple": al hacer clic en un paso sugerido, se auto-llenan la descripción, tipo (Correctiva), prioridad (Alta), fuente (Auditoría) y observaciones con el hallazgo de ese paso. La tabla muestra todas las acciones con tipo (badge color: rojo=Correctiva, azul=Preventiva, verde=Mejora), prioridad y estado en badges de colores. Para **actualizar el estado** de una acción ya creada: clic en la fila de la tabla → se abre panel de detalle → campo "Actualizar Estado" (opciones: Pendiente / En Proceso / Completada / Verificada) → botón "Guardar".
...
### PESV - Preguntas frecuentes sobre pasos exclusivos del LSO (P03, P05, P06, V03)

- **¿Cómo diligenciar el paso P03 - Diagnóstico / Contexto Organizacional?** El paso P03 es **exclusivo del LSO asignado** — la empresa no lo diligencia. Para que el LSO pueda gestionarlo, la empresa debe tener diligenciados los siguientes módulos: **H04 - Vehículos Seguros**, **H01 - Factor Humano / Conductores**, **H03 - Comparendos** (infracciones de cada conductor en H01), **H02 - Capacitación en Seguridad Vial**, **V02 - Registro y Análisis de Siniestros**, y el módulo **Trabajadores** del SG-SST. Orden sugerido: H04 → H01 → H03 → H02 → V02 → Trabajadores → luego el LSO gestiona P03.
...
- **¿Qué módulos necesita la empresa para que el LSO gestione P03?** El paso P03 es exclusivo del LSO. Para que el LSO pueda gestionarlo, la empresa debe tener diligenciados: **H04 - Vehículos Seguros**, **H01 - Factor Humano / Conductores**, **H03 - Comparendos** (infracciones de cada conductor en H01), **H02 - Capacitación en Seguridad Vial**, **V02 - Registro y Análisis de Siniestros**, y el módulo **Trabajadores** del SG-SST.

- **¿Cómo diligenciar el paso P05 - Objetivos e Indicadores?** El paso P05 es **exclusivo del LSO asignado** — la empresa no lo diligencia. Para que el LSO pueda gestionarlo, la empresa debe tener diligenciados: **H04 - Vehículos Seguros** (flota con SOAT y RTM), **H01 - Factor Humano / Conductores** (conductores activos con fechas de licencia y examen médico), **H03 - Comparendos** (infracciones de cada conductor), **H02 - Capacitación en Seguridad Vial**, **H05 - Mantenimiento Vehicular**, **H06 - Inspecciones Preoperacionales**, y **V02 - Registro y Análisis de Siniestros**. Orden sugerido: H04 → H01 → H03 → H02 → H05 → H06 → V02 → luego el LSO gestiona P05.

- **¿Qué módulos necesita la empresa para que el LSO gestione P05?** El paso P05 es exclusivo del LSO. Para que el LSO pueda gestionarlo, la empresa debe tener diligenciados: **H04 - Vehículos Seguros**, **H01 - Factor Humano / Conductores** (con fechas de licencia y examen médico de cada conductor), **H03 - Comparendos**, **H02 - Capacitación en Seguridad Vial**, **H05 - Mantenimiento Vehicular**, **H06 - Inspecciones Preoperacionales**, y **V02 - Registro y Análisis de Siniestros**.

- **¿Cómo diligenciar el paso P06 - Factores de Desempeño SPF?** El paso P06 es **exclusivo del LSO asignado** — la empresa no lo diligencia. Para que el LSO pueda gestionarlo, la empresa debe tener diligenciados: **H04 - Vehículos Seguros** (flota con SOAT y RTM), **H01 - Factor Humano / Conductores** (conductores activos con fechas de licencia y examen médico), **H03 - Comparendos** (infracciones de cada conductor), **H02 - Capacitación en Seguridad Vial**, **H05 - Mantenimiento Vehicular**, **H06 - Inspecciones Preoperacionales**, y **V02 - Registro y Análisis de Siniestros**. Orden sugerido: H04 → H01 → H03 → H02 → H05 → H06 → V02 → luego el LSO gestiona P06.

- **¿Qué módulos necesita la empresa para que el LSO gestione P06?** El paso P06 es exclusivo del LSO. Para que el LSO pueda gestionarlo, la empresa debe tener diligenciados: **H04 - Vehículos Seguros**, **H01 - Factor Humano / Conductores**, **H03 - Comparendos**, **H02 - Capacitación en Seguridad Vial**, **H05 - Mantenimiento Vehicular**, **H06 - Inspecciones Preoperacionales**, y **V02 - Registro y Análisis de Siniestros**.

- **¿Los pasos P03, P05, P06 y V03 los puede diligenciar la empresa directamente?** No. Estos pasos son **exclusivos del Licenciado en SST (LSO) asignado** a la empresa. La empresa verá el panel "En espera del Licenciado en SST" mientras el LSO no los haya completado. Lo que puede hacer la empresa es asegurarse de tener los módulos de soporte diligenciados (vehículos, conductores, capacitaciones, siniestros, etc.) para que cuando el LSO ingrese, el sistema tenga datos reales disponibles.
