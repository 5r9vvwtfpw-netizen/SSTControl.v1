...
const INDEXES = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_acciones_mejora_company_id ON acciones_mejora (company_id)`,
  `CREATE INDEX IF NOT EXISTS idx_acciones_mejora_evaluacion_id ON acciones_mejora (evaluacion_id)`,
  `CREATE INDEX IF NOT EXISTS idx_actividades_plan_trabajo_company_id ON actividades_plan_trabajo (company_id)`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_licensed_professional_assignments_professional_id ON licensed_professional_assignments (professional_id)`,
  ...
  for (const ddl of INDEXES) {
    try {
      ...
      if (error.message?.includes("already exists")) {
        ...
        const match = ddl.match(/idx_(\w+)/);
        console.warn(`[Migration] Skipping index ${match?.[1] || 'unknown'}: ${error.message?.slice(0, 80)}`);
        skipped++;
      }
    }
  }
  console.log(`[Migration] Performance indexes: ${created} created, ${skipped} skipped.`);
}
