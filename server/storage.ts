...
export interface IStorage {
  ...
  addMatrizLegalMissingEntries(companyId: string): Promise<number>;
...
export class DbStorage implements IStorage {
  ...
  async deleteUser(id: string): Promise<void> {
    await db.transaction(async (tx) => {
      // Dynamic approach: query all foreign keys referencing users.id from the database catalog
      // This ensures we never miss a table, even when new tables are added
      const fkRefs = await tx.execute(sql`
        SELECT 
          tc.table_name,
          kcu.column_name,
          c.is_nullable
        ...
        ORDER BY c.is_nullable ASC, tc.table_name
      `);

      // Process each foreign key reference dynamically
      for (const ref of fkRefs.rows as Array<{ table_name: string; column_name: string; is_nullable: string }>) {
        const { table_name, column_name, is_nullable } = ref;
        if (is_nullable === 'NO') {
          // NOT NULL column - must delete the rows
          await tx.execute(sql.raw(`DELETE FROM "${table_name}" WHERE "${column_name}" = '${id}'`));
        } else {
          // Nullable column - set to NULL
          await tx.execute(sql.raw(`UPDATE "${table_name}" SET "${column_name}" = NULL WHERE "${column_name}" = '${id}'`));
        }
      }
  ...
  /**
   * Agrega normas faltantes a empresas que ya tienen la matriz inicializada.
   * Idempotente — verifica por texto de norma antes de insertar.
   */
  async addMatrizLegalMissingEntries(companyId: string): Promise<number> {
    const existing = await this.getMatrizLegal(companyId);
    if (existing.length === 0) return 0; // Empresa sin matriz: usar initializeMatrizLegal

    const existingNormas = new Set(existing.map(e => e.norma));
    let added = 0;

    const nuevasNormas: Partial<schema.InsertMatrizLegal>[] = [
      {
        norma: "Resolución 2346 de 2007 – Art. 4 y 5",
        fechaEmision: new Date("2007-07-11"),
        entidadEmisora: "Ministerio de la Protección Social",
        categoria: "medicina-trabajo",
        titulo: "Profesiograma – Perfil de Exámenes Médicos por Cargo",
        descripcion: "El profesiograma define para cada cargo los exámenes médicos ocupacionales requeridos según la exposición a factores de riesgo (Art. 4 y 5 Res. 2346/2007).",
        obligaciones: "Elaborar y mantener actualizado el profesiograma, definiendo para cada cargo los exámenes médicos de ingreso, periódicos y retiro según los factores de riesgo del puesto.",
        articulosAplicables: "Art. 4, Art. 5, Art. 7",
        alcance: "Todos los empleadores",
        periodicidad: "Actualización anual o ante cambios en el cargo",
      ...
      },
      {
        ...
        categoria: "sistema-gestion",
        titulo: "Organigrama del SG-SST – Roles y Responsabilidades",
        descripcion: "El empleador debe definir y comunicar la estructura organizativa del SG-SST con roles, responsabilidades y autoridades en materia de SST.",
        obligaciones: "Documentar y divulgar el organigrama del SG-SST con los roles de: Representante Legal, Responsable SST, COPASST o Vigía, Brigadas de Emergencia y demás responsables preventivos.",
        articulosAplicables: "Art. 2.2.4.6.8, Art. 2.2.4.6.9",
        alcance: "Todos los empleadores",
        periodicidad: "Actualización ante cambios organizacionales",
    ...
    for (const norma of nuevasNormas) {
      if (!existingNormas.has(norma.norma!)) {
        await this.createMatrizLegalItem(norma as schema.InsertMatrizLegal, companyId);
        added++;
      }
    }
