...
export interface IStorage {
  ...
  // ========================================
  // Dashboard PLANEAR - Planificación SST (PHVA cycle)
  // ========================================
  getDashboardPlanear(companyId: string): Promise<{
    evaluaciones: {
      total: number;
      ultimaEvaluacion: number | null;
      fechaUltimaEvaluacion: string | null;
...
export class DbStorage implements IStorage {
  ...
  // Dashboard PLANEAR - Planificación SST (PHVA cycle)
  // ========================================
  async getDashboardPlanear(companyId: string): Promise<{
    evaluaciones: {
      total: number;
      ultimaEvaluacion: number | null;
      fechaUltimaEvaluacion: string | null;
    ...
    const totalEvaluaciones = allEvaluaciones.length;
    const ultimaEvaluacion = allEvaluaciones.length > 0 ? allEvaluaciones[0] : null;
    const penultimaEvaluacion = allEvaluaciones.length > 1 ? allEvaluaciones[1] : null;
    ...
    const tendenciaMejora = ultimaEvaluacion && penultimaEvaluacion
      ? (ultimaEvaluacion.puntajeTotal ?? 0) > (penultimaEvaluacion.puntajeTotal ?? 0)
      : false;

    // ========== PLAN DE TRABAJO ANUAL ==========
    const planesActivos = await db.select()
      .from(schema.planesTrabajoAnual)
      .where(and(
        eq(schema.planesTrabajoAnual.companyId, companyId),
        eq(schema.planesTrabajoAnual.anio, currentYear)
    ...
    const planActivo = planesActivos.length > 0 ? planesActivos[0] : null;
    ...
    const objetivosPorVencer = allObjetivos.filter(o => 
      ...
      o.fechaMeta >= todayStr
    ).length;

    // ========== MATRIZ LEGAL ==========
    const allRequisitos = await db.select()
      .from(schema.matrizLegal)
      .where(eq(schema.matrizLegal.companyId, companyId));
    ...
    // ========== PELIGROS (MATRIZ IPERC) ==========
    const matricesIperc = await db.select()
    ...
    // ========== POLÍTICAS SST ==========
    const allPoliticas = await db.select()
      .from(schema.politicasSst)
      .where(eq(schema.politicasSst.companyId, companyId));
    ...
    return {
      evaluaciones: {
        ...
        ultimaEvaluacion: ultimaEvaluacion?.puntajeTotal ?? null,
        fechaUltimaEvaluacion: ultimaEvaluacion?.fechaEvaluacion ?? null,
        estandaresCriticos,
        tendenciaMejora
      },
      planTrabajo: {
        planesActivos: planesActivos.length,
        porcentajeCumplimiento: planActivo?.porcentajeCumplimiento ?? 0,
        actividadesCompletadas,
      ...
      objetivos: {
        ...
        activos: objetivosActivos,
        cumplidos: objetivosCumplidos,
        porVencer: objetivosPorVencer
      },
      matrizLegal: {
        requisitosIdentificados,
        actualizadosEsteAnio
      },
      peligros: {
        identificados: peligrosIdentificados,
        altoRiesgo: peligrosAltoRiesgo,
        controlados: peligrosControlados
      },
      politicas: {
        vigentes: politicasVigentes,
        porActualizar: politicasPorActualizar
      }
    };
  }
