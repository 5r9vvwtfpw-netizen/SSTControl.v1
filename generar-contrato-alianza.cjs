/**
 * GENERADOR DE ALIANZA — SST Colombia
 * 
 * Uso: node generar-alianza-alianza.js
 * Salida: alianza-alianza.pdf (en la raíz del proyecto)
 * 
...
const MARGIN = 50;
const OUTPUT = path.join(__dirname, 'alianza-sst.pdf');
...
const doc = new PDFDocument({
  ...
  margin: MARGIN,
  info: {
    Title: 'Alianza y Licenciamiento de Software — SST Colombia',
    Author: 'SISTEMA AUTOMATIZADO DE GESTIÓN INTEGRAL S.A.S.',
    Subject: 'Alianza de Prestación de Servicios y Licenciamiento',
...
doc.fontSize(22).font('Helvetica-Bold').fillColor(C.WHITE)
   .text('ALIANZA DE PRESTACIÓN DE', MARGIN, 220, { width: pw - MARGIN * 2, align: 'center' });
...
bullet(doc, 'Información Confidencial', 'Toda información técnica, comercial, financiera, de clientes o estratégica de EL PROVEEDOR a la que EL CONSULTOR tenga acceso con ocasión de esta alianza, incluyendo sin limitación: datos de clientes, metodologías, precios, estructura de La Plataforma y la Lógica de Negocio.');
...
bullet(doc, '', 'Mantener la Información Confidencial en reserva estricta durante la vigencia de la alianza y por cinco (5) años adicionales contados desde su terminación.');
...
bullet(doc, '', 'No subcontratar ni delegar las obligaciones derivadas de esta alianza sin autorización escrita previa de EL PROVEEDOR.');
...
para(doc, `EL CONSULTOR se obliga a no participar, desarrollar, financiar, asesorar, ni prestar servicios directamente a plataformas tecnológicas de gestión SST que compitan directa o indirectamente con La Plataforma de EL PROVEEDOR, durante la vigencia de esta alianza y por un período de CUATRO (4) AÑOS contados desde la fecha de su terminación, en el territorio Colombiano.`);
...
para(doc, `La presente alianza tendrá vigencia a partir del 15 DE AGOSTO DE 2026 y una duración de UN (1) AÑO, prorrogable automáticamente por períodos iguales salvo que cualquiera de las partes notifique por escrito su intención de no renovar con al menos TREINTA (30) días de antelación.`);
subTitle(doc, 'Son causales de terminación por justa causa:');
bullet(doc, '', 'El incumplimiento grave de cualquier obligación establecida en esta alianza.');
...
para(doc, `La terminación de la alianza no extingue las obligaciones de confidencialidad, no competencia, no solicitud y cláusula penal, las cuales permanecen vigentes por los términos establecidos en cada cláusula respectiva.`);
...
para(doc, `La presente alianza se regirá e interpretará conforme a las leyes de Colombia, en particular: el Código de Comercio, el Código Civil, la Ley 23 de 1982 y sus modificaciones, en especial la Ley 1915 de 2018 (Derechos de Autor), la Decisión Andina 351 de la Comunidad Andina, la Ley 1581 de 2012 (Protección de Datos Personales), la Ley 527 de 1999 (Comercio Electrónico), y las disposiciones de Propiedad Industrial contenidas en la Decisión Andina 486.`);

// ── CLÁUSULA DÉCIMA CUARTA ───────────────────────────────────────────────────
clauseTitle(doc, 'DÉCIMA CUARTA', 'Disposiciones Generales');
bullet(doc, 'Integralidad', 'Esta alianza constituye el acuerdo completo entre las partes respecto a su objeto y reemplaza cualquier acuerdo previo, verbal o escrito, sobre la misma materia.');
bullet(doc, 'Modificaciones', 'Cualquier modificación a esta alianza deberá constar por escrito y ser suscrita por los representantes autorizados de ambas partes.');
bullet(doc, 'Nulidad parcial', 'Si alguna cláusula de esta alianza fuere declarada nula o inaplicable, las demás cláusulas permanecerán vigentes en su totalidad.');
bullet(doc, 'Independencia', 'EL CONSULTOR actúa como contratista independiente. Nada en esta alianza crea una relación laboral, sociedad, joint venture o agencia entre las partes.');
...
checkPage(doc, 160);
doc.moveDown(1);
para(doc, `En señal de conformidad con todas y cada una de las cláusulas anteriores, las partes suscriben la presente alianza en la ciudad de Medellín, Antioquia, el día 15 del mes de agosto de 2026.`);
