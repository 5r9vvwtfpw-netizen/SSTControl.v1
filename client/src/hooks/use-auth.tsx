    ...
    if (typeof BroadcastChannel !== "undefined") {
      ...
      authChannelRef.current.onmessage = (event) => {
        if (event.data?.type === "LOGOUT") {
          ...
          // No redirigir si estamos en un portal independiente (LSO, empleados, soporte).
          // Esos portales tienen su propia sesión y no deben verse afectados
          // por el logout de la sesión principal.
          const path = window.location.pathname;
          const STANDALONE_PORTALS = ["/portal-licenciado", "/portal-empleados", "/soporte"];
          const isStandalonePortal = STANDALONE_PORTALS.some(p => path.startsWith(p));
...
// ─── Portal de Empleados auth (sst_emp session) ───────────────────────────
export function useEmpAuth() {
  const { data: user, isLoading } = useQuery<SelectUser | undefined, Error>({
    queryKey: ["/api/emp/user"],
    queryFn: getQueryFn({ on401: "returnNull" }),
  });
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: (userData: SelectUser) => {
      queryClient.setQueryData(["/api/emp/user"], userData);
    },
    onError: (error: Error) => {
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      await apiRequest("POST", "/api/emp/logout");
    },
    onSuccess: () => {
      queryClient.setQueryData(["/api/emp/user"], null);
      window.location.reload();
    },
  });

  return { user: user ?? null, isLoading, loginMutation, logoutMutation };
}

// ─── Portal Licenciado (LSO) auth (sst_lso session) ──────────────────────
export function useLsoAuth() {
  const { data: user, isLoading } = useQuery<SelectUser | undefined, Error>({
  ...
  const loginMutation = useMutation({
    mutationFn: async (credentials: { username: string; password: string }) => {
      const res = await fetch("/api/lso/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(credentials),
        credentials: "include",
    ...
    onSuccess: (userData: SelectUser) => {
      queryClient.setQueryData(["/api/lso/user"], userData);
    },
    onError: (error: Error) => {
      toast({ title: "Error de inicio de sesión", description: error.message, variant: "destructive" });
    },
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      await apiRequest("POST", "/api/lso/logout");
    },
    onSuccess: () => {
      queryClient.setQueryData(["/api/lso/user"], null);
      window.location.reload();
    },
  });
