...
1. **Al decodificar el JWT en `/auth?quote={jwt}&tab=register`**, extraer el campo `company_name` y mostrar la pestaña "Registrarse":

> **IMPORTANTE**: El parámetro `tab=register` indica que se debe mostrar la pestaña de registro, no la de inicio de sesión.
...
El endpoint `/auth?quote={JWT}&tab=register` debe:

## ⚠️ CAMPOS OBLIGATORIOS A PRE-LLENAR

El usuario YA ingresó estos datos en sst-colombia.com.co. **NO debe volver a ingresarlos manualmente**. El JWT contiene toda la información necesaria.

### Tabla de Mapeo de Campos

| Campo en sst-colombia.com | Valor del JWT | Dónde Pre-llenar |
|---------------------------|---------------|------------------|
| **Nombre de empresa** | `metadata.company_name` | Registro + Paso 2 |
| **Número de Trabajadores** | `metadata.employees` | Paso 1 + /empresa |
| **Nivel de Riesgo** | `metadata.risk_level` | Paso 1 + /empresa |
| **Número de Vehículos (PESV)** | `metadata.vehicles` | Paso 1 + /empresa |
| **Cupón de Descuento** | `metadata.coupon_code` | Badge visible (no editable) |

### ⚠️ IMPORTANTE: Persistencia en Base de Datos

Cuando el usuario hace click en "Crear Empresa y Comenzar Prueba", los datos del JWT deben guardarse en la tabla `companies`:
...
Luego, la página `/empresa` debe mostrar estos valores desde la base de datos.
...
2. **Paso 1: Actividad Económica y Trabajadores** - Pre-llenar:
   ...
   - `Nivel de Riesgo` ← `metadata.risk_level` (II = "Riesgo Bajo", III = "Riesgo Medio", etc.)
   - `Vehículos (PESV)` ← `metadata.vehicles`

3. **Paso 2: Datos de la Empresa** - Pre-llenar:
   - `Nombre de la Empresa` ← `metadata.company_name` (OBLIGATORIO - no debe estar vacío)

4. **Página /empresa (Editar Empresa)** - Pre-llenar al crear la empresa:
   - `Número de Vehículos (PESV)` ← `metadata.vehicles` (debe guardarse en la tabla `companies` y mostrarse aquí)
   - `Número de Trabajadores` ← `metadata.employees`
   - `Nivel de Riesgo` ← `metadata.risk_level`

5. **Cupón** - Mostrar como aplicado:
   - Si `metadata.coupon_code` existe, mostrar el badge del cupón aplicado
   - NO permitir cambiar el cupón (ya fue validado y los precios dependen de él)
...
app.get('/auth', (req, res) => {
  ...
  req.session.prefillData = {
    companyName: decoded.metadata.company_name,
    employees: decoded.metadata.employees,
    riskLevel: decoded.metadata.risk_level,      // ← IMPORTANTE
    vehicles: decoded.metadata.vehicles,          // ← IMPORTANTE
    couponCode: decoded.metadata.coupon_code      // ← IMPORTANTE
  };
...
// En Paso 1
<input name="employees" type="number" defaultValue={prefill.employees || 1} />
<select name="riskLevel" defaultValue={prefill.riskLevel || 'I'}>
...
<input name="vehicles" type="number" defaultValue={prefill.vehicles || 0} />

// Mostrar cupón aplicado (NO editable)
{prefill.couponCode && (
  <div className="coupon-badge">
    <span>Cupón aplicado: {prefill.couponCode}</span>
  </div>
)}
```
...
El usuario en sst-colombia.com.co ingresó:
- Nombre: "Cie22"
- Empleados: 2
- Nivel de Riesgo: II - Riesgo Bajo
- Vehículos: 2
- Cupón: Q7KKHTSP

En sst-colombia.com, TODOS estos campos deben aparecer pre-llenados:

```
┌─────────────────────────────────────────┐
│ Nombre de empresa: [Cie22          ]    │  ← Pre-llenado
│ Número de Trabajadores: [2         ]    │  ← Pre-llenado
│ Nivel de Riesgo: [II - Riesgo Bajo ▼]   │  ← Pre-llenado
│ Vehículos (PESV): [2               ]    │  ← Pre-llenado
│ Cupón: [Q7KKHTSP] ✓ Aplicado            │  ← Mostrar como badge
└─────────────────────────────────────────┘
```
...
## Webhook para Notificar Uso de Cupones

Cuando un usuario completa su suscripción usando un cupón, **sst-colombia.com debe notificar a sst-colombia.com.co** para actualizar el contador de usos del cupón.

### Endpoint del Webhook

```
POST https://sst-colombia.com.co/api/webhooks/coupon-redeemed
```
...
| `X-Webhook-Secret` | `{WEBHOOK_SECRET}` | Secreto compartido para autenticación |

### Body del Request

```json
{
  "coupon_code": "IK9AZIYN",
...
### Cuándo Llamar el Webhook

Llamar este webhook cuando:
1. La suscripción de Stripe se crea exitosamente
2. El primer pago es confirmado (para cupones que no son 100% gratis)

### Ejemplo de Implementación

```javascript
// Después de crear la suscripción exitosamente
async function notifyCouponRedemption(couponCode, companyId, subscriptionId) {
  try {
    const response = await fetch('https://sst-colombia.com.co/api/webhooks/coupon-redeemed', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Webhook-Secret': process.env.LANDING_WEBHOOK_SECRET,
      },
      body: JSON.stringify({
    ...
    if (!response.ok) {
      console.error('Failed to notify coupon redemption:', await response.text());
    }
  } catch (error) {
    console.error('Error notifying coupon redemption:', error);
    // Non-blocking: subscription still works even if webhook fails
  }
}

// Usar después de crear la suscripción
await stripe.subscriptions.create({ ... });
await notifyCouponRedemption(
  quoteData.metadata.coupon_code,
  newCompany.id,
...
El secreto debe configurarse como variable de entorno en sst-colombia.com:

```
LANDING_WEBHOOK_SECRET=<mismo valor que LSO_REGISTRY_API_KEY en sst-colombia.com.co>
```
