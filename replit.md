...
The Support Portal includes a real-time internal chat system for agent coordination, utilizing WebSocket for role-restricted broadcasts. The team chat is available as a floating window in the support portal layout (FloatingTeamChat component). The chat supports: @mentions with autocomplete dropdown (typing "@" shows online agents, selecting inserts the mention, mentioned users receive notifications), unread message indicators per channel (red counter badges on channels with new messages, total unread badge on the closed chat button), file/image uploads via a paperclip button (images render inline, other files as download links, stored in S3 object storage), and a persistent channel sidebar that stays open when switching channels. The `support_chat_messages` table includes `attachments` (text array of JSON-serialized file metadata) and `mentions` (text array of mentioned user IDs). Support tickets have an archive system: closed/resolved tickets are hidden from the main view by default and accessible via a "Ver archivo" toggle button. Auto-assignment assigns tickets to the first agent who responds publicly.
