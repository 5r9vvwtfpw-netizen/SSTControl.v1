...
Tener la integración general de GitHub activa y contar con `push: true` en su API no configura automáticamente las credenciales del remoto Git de la shell; un `git push` puede seguir fallando hasta que esté activa la conexión **Source Control** del editor.