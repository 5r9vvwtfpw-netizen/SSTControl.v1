...
// ==========================================
// ACCIONES CORRECTIVAS Y PREVENTIVAS (Estándar 7.1.1)
// Decreto 1072/2015 Art. 2.2.4.6.33
// ==========================================
export const tipoAccionCorrectivaEnum = pgEnum("tipo_accion_correctiva", [
  "correctiva",
  "preventiva",
  "mejora"
]);

export const origenAccionCorrectivaEnum = pgEnum("origen_accion_correctiva", [
  "auditoria_interna",
  "revision_direccion",
  "inspeccion",
...
export const estadoAccionCorrectivaEnum = pgEnum("estado_accion_correctiva", [
  "abierta",
  "en_proceso",
  "implementada",
  "verificada",
...
export const accionesCorrectivas = pgTable("acciones_correctivas", {
  ...
  // Identificación
  codigo: text("codigo").notNull(), // Código único ej: "AC-2025-001"
  tipo: tipoAccionCorrectivaEnum("tipo").notNull(),
  
  // Origen y vinculación
  origen: origenAccionCorrectivaEnum("origen").notNull(),
  origenDescripcion: text("origen_descripcion"), // Descripción del origen
  auditoriaInternaId: varchar("auditoria_interna_id").references(() => auditoriasInternas.id, { onDelete: "set null" }),
  revisionDireccionId: varchar("revision_direccion_id").references(() => revisionesDireccion.id, { onDelete: "set null" }),
  noConformidadId: varchar("no_conformidad_id"), // Referencia a no_conformidades (se vinculará después)
  
  // Descripción del problema
  hallazgo: text("hallazgo").notNull(), // Descripción del problema/hallazgo
  causaRaiz: text("causa_raiz"), // Análisis de causa raíz
  
  // Acción propuesta
  accionPropuesta: text("accion_propuesta").notNull(),
  resultadoEsperado: text("resultado_esperado"),
  ...
  // Seguimiento
  estado: estadoAccionCorrectivaEnum("estado").notNull().default("abierta"),
  porcentajeAvance: integer("porcentaje_avance").notNull().default(0),
  prioridad: prioridadMejoraEnum("prioridad").notNull().default("media"),
  
  // Verificación de eficacia
  eficazVerificada: integer("eficaz_verificada").default(0), // 0 = no, 1 = sí
  verificadoPorId: varchar("verificado_por_id").references(() => users.id),
  observacionesVerificacion: text("observaciones_verificacion"),
  
  // Evidencias
  evidenciaImplementacion: text("evidencia_implementacion"),
  evidenciaEficacia: text("evidencia_eficacia"),
...
export const insertAccionCorrectivaSchema = createInsertSchema(accionesCorrectivas)
  .omit({ id: true, createdAt: true, updatedAt: true, companyId: true })
  .extend({
    fechaDeteccion: z.coerce.date(),
    fechaLimite: z.coerce.date(),
...
export type InsertAccionCorrectiva = z.infer<typeof insertAccionCorrectivaSchema>;
export type AccionCorrectiva = typeof accionesCorrectivas.$inferSelect;

// ==========================================
// NO CONFORMIDADES (Estándar 7.1.1)
// ISO 45001:2018 Numeral 10.2
// ==========================================
export const tipoNoConformidadEnum = pgEnum("tipo_no_conformidad", [
  "mayor",
  "menor",
  "observacion"
]);

export const origenNoConformidadEnum = pgEnum("origen_no_conformidad", [
  "auditoria_interna",
  "auditoria_externa",
  "inspeccion",
  "revision_direccion",
...
export const estadoNoConformidadEnum = pgEnum("estado_no_conformidad", [
  "abierta",
  "en_analisis",
  "accion_definida",
  "en_implementacion",
...
export const noConformidades = pgTable("no_conformidades", {
  ...
  // Identificación
  codigo: text("codigo").notNull(), // Código único ej: "NC-2025-001"
  tipo: tipoNoConformidadEnum("tipo").notNull(),
  
  // Origen y referencia
  origen: origenNoConformidadEnum("origen").notNull(),
  requisitoIncumplido: text("requisito_incumplido"), // Requisito legal o normativo incumplido
  normaReferencia: text("norma_referencia"), // ISO 45001, Decreto 1072, etc.
  auditoriaInternaId: varchar("auditoria_interna_id").references(() => auditoriasInternas.id, { onDelete: "set null" }),
  
  // Descripción
  descripcion: text("descripcion").notNull(), // Descripción detallada de la NC
  evidenciaObjetiva: text("evidencia_objetiva"), // Evidencia objetiva del incumplimiento
  areaProceso: text("area_proceso"), // Área o proceso afectado
  
  // Análisis
  causaRaiz: text("causa_raiz"),
  metodologiaAnalisis: text("metodologia_analisis"), // 5 Por qué, Ishikawa, etc.
  
  // Responsables
  detectadoPorId: varchar("detectado_por_id").references(() => users.id),
  detectadoPorNombre: text("detectado_por_nombre"),
  responsableAreaId: varchar("responsable_area_id").references(() => users.id),
  responsableAreaNombre: text("responsable_area_nombre"),
  
  // Fechas
  fechaDeteccion: date("fecha_deteccion").notNull(),
  fechaLimiteAnalisis: date("fecha_limite_analisis"),
  fechaCierre: date("fecha_cierre"),
  
  // Estado y seguimiento
  estado: estadoNoConformidadEnum("estado").notNull().default("abierta"),
  prioridad: prioridadMejoraEnum("prioridad").notNull().default("media"),
  
  // Tratamiento inmediato
  correccionInmediata: text("correccion_inmediata"), // Corrección inmediata aplicada
  fechaCorreccion: date("fecha_correccion"),
  
  // Cierre
  verificadaCierre: integer("verificada_cierre").default(0),
  verificadaPorId: varchar("verificada_por_id").references(() => users.id),
  observacionesCierre: text("observaciones_cierre"),
...
export const insertNoConformidadSchema = createInsertSchema(noConformidades)
  .omit({ id: true, createdAt: true, updatedAt: true, companyId: true })
  .extend({
    fechaDeteccion: z.coerce.date(),
    fechaLimiteAnalisis: z.coerce.date().optional().nullable(),
    fechaCierre: z.coerce.date().optional().nullable(),
    fechaCorreccion: z.coerce.date().optional().nullable(),
  });
export type InsertNoConformidad = z.infer<typeof insertNoConformidadSchema>;
export type NoConformidad = typeof noConformidades.$inferSelect;

// ==========================================
// OPORTUNIDADES DE MEJORA (Estándar 7.1.1)
// Decreto 1072/2015 Art. 2.2.4.6.34
// ==========================================
export const categoriaOportunidadMejoraEnum = pgEnum("categoria_oportunidad_mejora", [
  "proceso",
  "infraestructura",
  "capacitacion",
  "documentacion",
...
export const estadoOportunidadMejoraEnum = pgEnum("estado_oportunidad_mejora", [
  "identificada",
  "evaluada",
  "aprobada",
  "en_implementacion",
...
export const oportunidadesMejora = pgTable("oportunidades_mejora", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  
  // Identificación
  codigo: text("codigo").notNull(), // Código único ej: "OM-2025-001"
  titulo: text("titulo").notNull(),
  categoria: categoriaOportunidadMejoraEnum("categoria").notNull(),
  
  // Descripción
  descripcion: text("descripcion").notNull(),
  situacionActual: text("situacion_actual"), // Descripción de la situación actual
  mejoraEsperada: text("mejora_esperada"), // Beneficio esperado
  areaProceso: text("area_proceso"), // Área o proceso a mejorar
  
  // Origen
  fuenteIdentificacion: text("fuente_identificacion"), // Cómo se identificó: auditoria, sugerencia, indicador, etc.
  identificadoPorId: varchar("identificado_por_id").references(() => users.id),
  identificadoPorNombre: text("identificado_por_nombre"),
  
  // Evaluación
  impactoEstimado: text("impacto_estimado"), // Alto, Medio, Bajo o descripción
  recursosRequeridos: text("recursos_requeridos"),
  costoEstimado: integer("costo_estimado"), // En COP
  beneficioEstimado: text("beneficio_estimado"),
  ...
  // Fechas
  fechaIdentificacion: date("fecha_identificacion").notNull(),
  fechaEvaluacion: date("fecha_evaluacion"),
  fechaAprobacion: date("fecha_aprobacion"),
  fechaInicioImplementacion: date("fecha_inicio_implementacion"),
  fechaFinImplementacion: date("fecha_fin_implementacion"),
  fechaCierre: date("fecha_cierre"),
  
  // Estado y seguimiento
  estado: estadoOportunidadMejoraEnum("estado").notNull().default("identificada"),
  prioridad: prioridadMejoraEnum("prioridad").notNull().default("media"),
  porcentajeAvance: integer("porcentaje_avance").notNull().default(0),
  
  // Aprobación
  aprobadaPor: varchar("aprobada_por").references(() => users.id),
  motivoRechazo: text("motivo_rechazo"),
...
export const insertOportunidadMejoraSchema = createInsertSchema(oportunidadesMejora)
  .omit({ id: true, createdAt: true, updatedAt: true, companyId: true })
  .extend({
    fechaIdentificacion: z.coerce.date(),
    fechaEvaluacion: z.coerce.date().optional().nullable(),
    fechaAprobacion: z.coerce.date().optional().nullable(),
    fechaInicioImplementacion: z.coerce.date().optional().nullable(),
    fechaFinImplementacion: z.coerce.date().optional().nullable(),
    fechaCierre: z.coerce.date().optional().nullable(),
  });
export type InsertOportunidadMejora = z.infer<typeof insertOportunidadMejoraSchema>;
export type OportunidadMejora = typeof oportunidadesMejora.$inferSelect;
