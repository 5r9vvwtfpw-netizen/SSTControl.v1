...
// ==========================================
// TRAZABILIDAD INTEGRAL DEL SG-SST
// Sistema centralizado de hallazgos para trazabilidad completa
// Conforme a ISO 45001:2018 y Decreto 1072/2015
// ==========================================

export const moduloOrigenEnum = pgEnum("modulo_origen", [
  "evaluacion_0312",
  "auditoria_interna",
  "inspeccion",
...
export const hallazgosSistema = pgTable("hallazgos_sistema", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  
  codigoHallazgo: text("codigo_hallazgo").notNull(),
  
  moduloOrigen: moduloOrigenEnum("modulo_origen").notNull(),
  registroOrigenId: varchar("registro_origen_id"),
  registroOrigenTipo: text("registro_origen_tipo"),
  
  descripcion: text("descripcion").notNull(),
  evidencia: text("evidencia"),
  ubicacion: text("ubicacion"),
  
  severidad: severidadHallazgoEnum("severidad").notNull().default("media"),
  estado: estadoHallazgoEnum("estado").notNull().default("abierto"),
  
  objetivoSstId: varchar("objetivo_sst_id").references(() => objetivosSst.id, { onDelete: "set null" }),
  estandar0312Id: varchar("estandar_0312_id"),
  
  causaRaiz: text("causa_raiz"),
  analisisRealizado: text("analisis_realizado"),
  
  accionCorrectivaId: varchar("accion_correctiva_id"),
  accionPreventiva: text("accion_preventiva"),
  
  responsableId: varchar("responsable_id").references(() => users.id, { onDelete: "set null" }),
  responsableNombre: text("responsable_nombre"),
  
  fechaDeteccion: date("fecha_deteccion").notNull().default(sql`CURRENT_DATE`),
  fechaLimiteCorreccion: date("fecha_limite_correccion"),
  fechaCierre: date("fecha_cierre"),
  fechaVerificacion: date("fecha_verificacion"),
  
  eficaciaVerificada: integer("eficacia_verificada"),
  observacionesEficacia: text("observaciones_eficacia"),
  
  impactoIndicador: text("impacto_indicador"),
...
export const insertHallazgoSistemaSchema = createInsertSchema(hallazgosSistema)
  .omit({ id: true, createdAt: true, updatedAt: true, companyId: true })
  .extend({
    fechaDeteccion: z.coerce.date().optional(),
    fechaLimiteCorreccion: z.coerce.date().optional().nullable(),
    fechaCierre: z.coerce.date().optional().nullable(),
    fechaVerificacion: z.coerce.date().optional().nullable(),
  });
export type InsertHallazgoSistema = z.infer<typeof insertHallazgoSistemaSchema>;
export type HallazgoSistema = typeof hallazgosSistema.$inferSelect;

export const trazabilidadObjetivos = pgTable("trazabilidad_objetivos", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  companyId: varchar("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
  
  objetivoSstId: varchar("objetivo_sst_id").notNull().references(() => objetivosSst.id, { onDelete: "cascade" }),
  
  moduloVinculado: moduloOrigenEnum("modulo_vinculado").notNull(),
  registroVinculadoId: varchar("registro_vinculado_id").notNull(),
  registroDescripcion: text("registro_descripcion"),
  
  tipoVinculo: text("tipo_vinculo").notNull().default("contribuye"),
  porcentajeContribucion: integer("porcentaje_contribucion"),
  
  fechaVinculacion: date("fecha_vinculacion").notNull().default(sql`CURRENT_DATE`),
...
export const insertTrazabilidadObjetivosSchema = createInsertSchema(trazabilidadObjetivos)
  .omit({ id: true, createdAt: true, companyId: true })
  .extend({
    fechaVinculacion: z.coerce.date().optional(),
  });
export type InsertTrazabilidadObjetivos = z.infer<typeof insertTrazabilidadObjetivosSchema>;
export type TrazabilidadObjetivos = typeof trazabilidadObjetivos.$inferSelect;

export const MODULO_ORIGEN_LABELS: Record<string, string> = {
  evaluacion_0312: "Evaluación Resolución 0312/2019",
  auditoria_interna: "Auditoría Interna",
  inspeccion: "Inspección de Seguridad",
  investigacion_accidente: "Investigación de Accidentes",
  matriz_iperc: "Matriz IPERC",
  medicion_ambiental: "Medición Ambiental",
  ...
  contexto_organizacion: "Análisis de Contexto",
  otro: "Otro"
};

export const SEVERIDAD_HALLAZGO_LABELS: Record<string, { label: string; color: string }> = {
  critica: { label: "Crítica", color: "#dc2626" },
  alta: { label: "Alta", color: "#ea580c" },
  media: { label: "Media", color: "#ca8a04" },
  baja: { label: "Baja", color: "#16a34a" },
  observacion: { label: "Observación", color: "#6b7280" }
};
